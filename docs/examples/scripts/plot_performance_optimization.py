...
print(f"Grid size ratio: {len(source_lats)*len(source_lons) / (len(target_lats)*len(target_lons)):.1f}x")
...
print(f"Vectorization speedup: {single_regrid_time / (full_regrid_time / n_time_steps):.1f}x")
...
methods = ['Weight\nGeneration', 'Weight\nLoading', 'Single\nRegridding', 'Full Series\n(per step)']
times = [generation_time, loading_time, single_regrid_time, full_regrid_time / n_time_steps]
...
wedges, texts, autotexts = axes[0, 1].pie(
    memory_sizes, labels=memory_labels, autopct='%1.1f MB',
...
im1 = axes[1, 0].pcolormesh(
    source_lons, source_lats, dataset.temperature.isel(time=0),
...
im2 = axes[1, 1].pcolormesh(
    target_lons, target_lats, full_result.isel(time=0),
    shading='auto', cmap='RdYlBu_r'
)
