...
monsoon_asia = (
    6 * np.exp(-(((lons_2d - 120) / 30) ** 2)) * np.exp(-(((lats_2d - 25) / 15) ** 2))
)
monsoon_africa = (
    4 * np.exp(-(((lons_2d - 15) / 25) ** 2)) * np.exp(-(((lats_2d - 10) / 12) ** 2))
)
...
precip_noise = np.random.gamma(
    2, 0.5, lats_2d.shape
)  # Gamma distribution for precipitation
...
for i, month in enumerate(times):
    ...
    seasonal_monsoon = (
        6
        * np.exp(-(((lons_2d - 120) / 30) ** 2))
        * np.exp(-(((shifted_lat - 25) / 15) ** 2))
    )

    precip_data[i] = np.maximum(
        0, precip_base + seasonal_itcz + seasonal_monsoon + precip_noise
    )
...
print(
    f"Precipitation range: {precipitation.min().values:.1f} to {precipitation.max().values:.1f} mm/day"
)
...
for i, (method, data) in enumerate(july_data.items()):
    ...
    im = axes[0, i].pcolormesh(
        lons, lats, data, shading="auto", cmap="Blues", vmin=0, vmax=vmax
    )
...
# Plot differences
diff_conservative = precip_conservative.isel(time=july_idx) - precip_bilinear.isel(
    time=july_idx
)
...
for month in range(12):
    ...
    cons = calculate_total_precip(
        precip_conservative.isel(time=month).values, target_areas
    )
...
print("- Bilinear:     Intensive quantities (temperature, pressure, humidity)")
print("")
print("Key advantage of conservative regridding:")
print("- Preserves total integral (essential for flux conservation)")
