...
unstructured_grid = xr.Dataset({
    'lat': (['nCells'], unstructured_lats),
    'lon': (['nCells'], unstructured_lons)
})

print(f"Created unstructured grid with {n_cells} cells")
print(f"Latitude range: {unstructured_lats.min():.1f} to {unstructured_lats.max():.1f}°")
print(f"Longitude range: {unstructured_lons.min():.1f} to {unstructured_lons.max():.1f}°")
...
def create_atmospheric_field(lats, lons, field_type='temperature'):
    ...
    if field_type == 'temperature':
        ...
        topo_effect += -3 * np.exp(-((lons - 280)**2 + (lats - 45)**2) / 300)  # Rockies
        topo_effect += -4 * np.exp(-((lons - 350)**2 + (lats - 65)**2) / 200)  # Greenland
        ...
        # Subtropical jets
        jet_nh = 15 * np.exp(-((lats - 35) / 10)**2) * (1 + 0.3 * np.sin(3 * np.radians(lons)))
        jet_sh = 15 * np.exp(-((lats + 35) / 10)**2) * (1 + 0.3 * np.sin(3 * np.radians(lons)))
...
wind_data = create_atmospheric_field(
    unstructured_lats, unstructured_lons, 'wind_speed'
)
...
print(f"Temperature range: {temperature.min().values:.1f} to {temperature.max().values:.1f} °C")
print(f"Wind speed range: {wind_speed.min().values:.1f} to {wind_speed.max().values:.1f} m/s")
...
sc1 = axes[0, 0].scatter(
    unstructured_lons, unstructured_lats, c=temperature_data,
    s=1, cmap='RdYlBu_r', alpha=0.7
)
...
im1 = axes[0, 1].pcolormesh(
    target_lons, target_lats, temp_regridded,
...
density, _, _ = np.histogram2d(
    unstructured_lons, unstructured_lats,
    bins=[target_lons, target_lats]
)

im2 = axes[0, 2].pcolormesh(
    target_lons, target_lats, density.T,
...
sc2 = axes[1, 0].scatter(
    unstructured_lons, unstructured_lats, c=wind_data,
    s=1, cmap='plasma', alpha=0.7
)
...
im3 = axes[1, 1].pcolormesh(
    target_lons, target_lats, wind_regridded,
...
print(f"Unstructured → Structured regridding:")
...
print(f"- Unstructured grid density: ~{n_cells / (4 * np.pi * (180/np.pi)**2):.1f} points per degree²")
print(f"- Structured grid density: {len(target_lats) * len(target_lons) / (360 * 180):.1f} points per degree²")
print(f"- Effective resolution ratio: {(len(target_lats) * len(target_lons)) / n_cells:.1f}")

# Advantages of unstructured grids
print(f"\nAdvantages of unstructured grids:")
print(f"- Variable resolution capability")
print(f"- No polar singularities")
print(f"- Efficient for regional refinement")
print(f"- Better parallel scalability")
print(f"")
print(f"XRegrid advantages for unstructured data:")
print(f"- Automatic detection of grid type")
print(f"- Efficient ESMF LocStream handling")
print(f"- Same API for structured/unstructured grids")
print(f"- Optimized sparse matrix operations")
